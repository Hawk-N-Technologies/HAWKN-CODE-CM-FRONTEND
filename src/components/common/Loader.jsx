...
function Loader({ context = "section", label = "Loading…" }) {
  ...
  return (
    <div className={wrapperClass} role="status" aria-live="polite">
      <svg className={`${dim} animate-spin text-cm-blue-600`} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      {context !== "inline" && <span className="text-sm text-cm-text-muted">{label}</span>}
    </div>
  );
}
